...
//
// Board dimensions and initial position are configurable.
...
#[inline]
fn get_sq(pos: Pos, sq: usize) -> u32 { ((pos >> (sq * 4)) & 0xF) as u32 }
#[inline]
fn set_sq(pos: Pos, sq: usize, val: u32) -> Pos {
    (pos & !(0xFu128 << (sq * 4))) | ((val as Pos) << (sq * 4))
}
#[inline]
fn get_hand(pos: Pos, idx: usize) -> u32 { ((pos >> (HAND_SHIFT + idx * 2)) & 0x3) as u32 }
#[inline]
fn set_hand(pos: Pos, idx: usize, val: u32) -> Pos {
    (pos & !(0x3u128 << (HAND_SHIFT + idx * 2))) | ((val as Pos) << (HAND_SHIFT + idx * 2))
}

fn reflect_lr(pos: Pos) -> Pos {
...
#[inline]
fn canonical(pos: Pos) -> Pos {
    let r = reflect_lr(pos);
    if pos < r { pos } else { r }
}

fn flip_perspective(pos: Pos) -> Pos {
...
fn initial_position() -> Pos {
    let mut p: Pos = 0;
    // Black side (bottom)
    p = set_sq(p, sq_idx(4, 1), 1); // Lion
    p = set_sq(p, sq_idx(4, 2), 2); // Elephant
    p = set_sq(p, sq_idx(4, 0), 3); // Giraffe
    p = set_sq(p, sq_idx(3, 1), 4); // Chick
    // White side (top)
...
const HOME_ROW: usize = (ROWS - 1) as usize;
...
fn find_white_lion(pos: Pos) -> usize {
    for sq in 0..NUM_SQ {
...
fn black_can_reach(pos: Pos, target_sq: usize) -> bool {
...
fn merge_into(known: &mut Vec<Pos>, new_sorted: &[Pos]) {
...
fn diff_sorted(known: &[Pos], candidates: &[Pos], out: &mut Vec<Pos>) {
    out.clear();
    let mut ki = 0;
    for &c in candidates {
...
const BATCH_CAP: usize = 80_000_000;
...
fn main() {
    ...
    eprintln!("Phase 1 only: BFS reachable-position count.");

    let known = bfs();
    let n = known.len();
    let peak_rss_kb: Option<u64> = std::fs::read_to_string("/proc/self/status")
        .ok()
        .and_then(|s| {
            s.lines()
    ...
    if let Some(kb) = peak_rss_kb {
        println!("Peak RSS: {:.2} GiB", kb as f64 / 1_048_576.0);
    }
    println!("Note: fits in u32 index? {}", n <= u32::MAX as usize);
}
